...
def load_home_credit_data(max_rows: Optional[int] = None) -> pd.DataFrame:
    """
    Loads application_train.csv and engineers all features required across
    all 20 Streamlit analytical perspectives defined in Projects.ipynb.
    """
    ...
    if "TARGET" in df.columns:
        ...
        df["Target Label"] = df["TARGET"].map({0: "Repaid (TARGET = 0)", 1: "Default (TARGET = 1)"})
        df["Target Name"] = df["TARGET"].map({0: "Repaid", 1: "Default"})

    # 1. Derived Feature: Age and Age Cohorts
    ...
    # 2. Derived Feature: Employment Years (365243 is special code for pensioner/unemployed)
    ...
    # Annuity-to-Income Ratio (Repayment Burden)
    ...
    # 7. External Credit Bureau Composite Scores
    ...
    # 8. Clean Key Categorical Fields
    categorical_cols = [
