...
render_header(
    title="Page 14: Collateral Assets & Housing Security",
    subtitle="Evaluate the impact of vehicle ownership, real estate assets, and housing stability on credit default rates.",
...
c1.metric("Car Owners", format_number(car_owners), f"{(car_owners/total_clients*100):.1f}% of Applicants" if total_clients>0 else "")
c2.metric("Property Owners", format_number(realty_owners), f"{(realty_owners/total_clients*100):.1f}% of Applicants" if total_clients>0 else "")
c3.metric("Owns Both Assets", format_number(both_owners), "Dual Asset Security")
c4.metric("Property Owner Default", format_percent(realty_dr), "Realty Default Rate")

st.divider()

# Visualizations Row 1: Ownership Distributions
hs1, hs2 = st.columns(2)
with hs1:
    car_dist = filtered_df["FLAG_OWN_CAR"].value_counts().reset_index()
    car_dist.columns = ["Owns Car", "Count"]
    car_dist["Owns Car"] = car_dist["Owns Car"].map({"Y": "Yes (Car Owner)", "N": "No (No Car)"})
    fig_cardonut = create_donut_chart(car_dist, names_col="Owns Car", values_col="Count", title="Vehicle Ownership Distribution")
...
with hs2:
    ...
    realty_dist["Owns Property"] = realty_dist["Owns Property"].map({"Y": "Yes (Real Estate Owner)", "N": "No (Renter/Other)"})
    fig_realtydonut = create_donut_chart(realty_dist, names_col="Owns Property", values_col="Count", title="Real Estate Ownership Distribution")
    st.plotly_chart(fig_realtydonut, use_container_width=True)

# Visualizations Row 2: Default Rate by Car & Property
...
with hs3:
    ...
    car_dr["FLAG_OWN_CAR"] = car_dr["FLAG_OWN_CAR"].map({"Y": "Car Owner", "N": "No Car"})
    fig_cdr = create_bar_chart(car_dr, x_col="FLAG_OWN_CAR", y_col="Default Rate %", title="Default Rate % by Car Ownership")
...
with hs4:
    ...
    realty_dr_agg["FLAG_OWN_REALTY"] = realty_dr_agg["FLAG_OWN_REALTY"].map({"Y": "Property Owner", "N": "No Property"})
    fig_rdr = create_bar_chart(realty_dr_agg, x_col="FLAG_OWN_REALTY", y_col="Default Rate %", title="Default Rate % by Property Ownership")
    st.plotly_chart(fig_rdr, use_container_width=True)

# Visualizations Row 3: Housing Type Analysis
...
with hs5:
    ...
    fig_hdr = create_bar_chart(house_dr.sort_values("Default Rate %", ascending=False), x_col="NAME_HOUSING_TYPE", y_col="Default Rate %", title="Default Rate % by Housing Arrangement")
...
with hs6:
    ...
    fig_hcrd = create_bar_chart(house_crd, x_col="NAME_HOUSING_TYPE", y_col="AMT_CREDIT", title="Average Loan Amount by Housing Type")
