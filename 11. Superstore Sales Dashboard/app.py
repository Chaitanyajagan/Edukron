...
c1.metric("Total Applicants", format_number(kpis["total_applications"]), help="Total Loan Requests")
...
c3.metric("Avg Loan Amount", format_currency(kpis["avg_credit"]), "Mean Credit Issued")
c4.metric("Avg Client Income", format_currency(kpis["avg_income"]), "Annual Earnings")
c5.metric("Avg Applicant Age", f"{kpis['avg_age']:.1f} Yrs", f"Score: {kpis['avg_ext_score']:.2f}")
...
with r1:
    ...
    fig_target = create_donut_chart(target_counts, names_col="Status", values_col="Count", title="Loan Portfolio Default vs Repaid Share")
...
with r2:
    if "NAME_INCOME_TYPE" in df.columns:
        ...
        fig_inc = create_bar_chart(inc_agg.sort_values("Count", ascending=False).head(5), x_col="NAME_INCOME_TYPE", y_col="Default Rate %", title="Default Rate by Top Income Types", is_currency=False)
        st.plotly_chart(fig_inc, use_container_width=True)

st.divider()

# 20 Analytical Perspectives Matrix in pure Streamlit containers
st.subheader("🗂️ 20 Analytical Perspectives")
...
pages_info = [
    ...
    ("07 Credit Amount Analysis", "💳", "Loan sizes requested, credit brackets, and default rates by loan size."),
...
render_insights_card([
    ...
    f"**External Credit Bureau**: Average composite external score is **{kpis['avg_ext_score']:.3f}**, serving as a prime predictor of default propensity.",
